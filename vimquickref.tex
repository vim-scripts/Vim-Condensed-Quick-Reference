...
section \toright,\toleft~at \be		    \dotfill|]]|\w\w|[[|\w\w|][|\w\w|[]|\\
...
\subsection*{Marks}
mark current position, lsit marks	\dotfill|m|$c$\w\w|:marks |$[marks]$\\
goto local, global / \fst		\dotfill|`|$c$\w\w|`|$C$\w\w|'|$c$\w\w|'|$C$\\
before jump; last exit, edit, change	\dotfill|``|\w\w|`|$n$\w\w|`"|\w\w|`.|\\
\begend{operated text / \V}		\dotfill|`[|\w\w|`]|\w\w|`<|\w\w|`>|\\
...
set cursor to 1st line before interpreting 2nd	\dotfill|;|\\
...
lowercase, uppercase/not	    \dotfill|\l|\w\w|\u|\w\w|\L|\w\w|\U|\\
ignore case, force matching case    \dotfill|\c|\w\w|\C|\\
...
add char from the curr match for |/| \& |?| if |'is'| is on    \dotfill|^L|
\section*{Visual mode}
...
display unknown include files, all	\dotfill|:che|\OPT{ckpath}{!}
...
local, global identifier declaration		    \dotfill|gd|\w\w\w|gD|\\
the same, ignoring matches in sibling blocks	    \dotfill|1gd|\w\w|1gD|\\
